...
# def fast_agi_handler(config, reader, writer):
@asyncio.coroutine
def fast_agi_handler(reader, writer):
    # print(config)
    buffer = b''
    while b'\n\n' not in buffer:
        buffer += yield from reader.read(100)
    ...
    writer.write(b'EXEC StartMusicOnHold\n')
    buffer = b''
    while b'\n' not in buffer:
        buffer += yield from reader.read(100)
    message = buffer.decode()
...
if __name__ == '__main__':
    ...
    loop = asyncio.get_event_loop()
    # coro = asyncio.start_server(partial(fast_agi_handler, {'ex': 1}), '0.0.0.0', 4574, loop=loop)
    coro = asyncio.start_server(fast_agi_handler, '0.0.0.0', 4574, loop=loop)
